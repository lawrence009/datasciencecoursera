...
The adjustment resulted in `r ceiling(dmg.summary[2,1] / dmg.summary[1,1])`-fold improvement
in property damage estimates, or $`r ceiling((dmg.summary[2,1] - dmg.summary[1,1]) / 1e3)`
billions.
...
dtl[REFNUM == 605943, c('PROPDMG', 'PROPDMGEXP', 'CROPDMG', 'CROPDMGEXP'), with=F]
```
...
#HAIL
dtl[grepl('^hail', EVTYPE), EVTYPE:='Hail']

#HIGH WIND
dtl[grepl('^hight wind', EVTYPE), EVTYPE:='High Wind']

#HURRICANE
dtl[grepl('^hurricane', EVTYPE), EVTYPE:='Hurricane (Typhoon)']
```
... however matching EVTYPE and maintaining the code this way gets tedious quickly.
A pragmatic alternative approach is to alphabetically sort and save the list
unique EVTYPE to a csv file first.

```{r write foo.csv}
write.csv(sort(dtl[, unique(EVTYPE)]), file = 'foo.csv')
```
...
#What percent of EVTYPE is not mapped
sum(is.na(evmap)) / length(evtype)

#append the official event name
dtl <- merge(dtl, evmap, by = 'EVTYPE', all.x = T)

head(dtl[, c('EVTYPE', 'EventName'), with=F])
```

If ambiguous, EVTYPE is cross-reference against REMARKS.

```{r grepl EVTYPE REMARK, results='hide'}
## EVTYPE summary...

#HUNDERSTORM WIND
dtl[grepl('summary', EVTYPE) & grepl('[Tt]hunderstorm.*[Ww]ind', REMARKS), EventName:='Thunderstorm Wind']

#HAIL
dtl[grepl('summary', EVTYPE) & grepl('[Hh]ail', REMARKS), EventName:='Hail']

#FLASH FLOOD
dtl[grepl('summary', EVTYPE) & grepl('[Ff]lash flood', REMARKS), EventName:='Flood']

#LIGHTNING
dtl[grepl('summary', EVTYPE) & grepl('[Ll]ightning', REMARKS), EventName:='Lightning']

#BLIZZARD
dtl[grepl('summary', EVTYPE) & grepl('[Bb]lizzard', REMARKS), EventName:='Blizzard']

```

#### Summary of EVTYPE extraction

```{r mapping assessment}

dmg.notmapped <- dtl[is.na(EventName),
...
dmg.mapped <- dtl[!is.na(EventName),
                  .(fatalities=sum(FATALITIES, na.rm = T),
                    injuries=sum(INJURIES, na.rm = T),
                    proddmg=sum(PropDmg, na.rm = T),
                    cropdmg=sum(CropDmg, na.rm = T))]
...
Weather events can also be further reduced using key weather elements to enhance
the analysis.

```{r elements}
summary(evlist)

dtl <- merge(dtl, evlist, by = 'EventName', all.x = T)

summary(dtl[, c('EVTYPE', 'EventName', 'Element1', 'Element2'), with=F])
...
The region-to-state mapping extract from BEA can be found [here](https://github.com/lawrence009/datasciencecoursera/blob/master/repdata/peer2/BureauOfEconomicAnalysisRegions.csv).
```{r BEA US regions}
...
dtl <- merge(dtl, regions, by = 'STATE', all.x = T)

str(dtl[, c('STATE', 'Region'), with=F])
```
