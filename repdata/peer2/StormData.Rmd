...
propdmg <- dts[bad.propexp, .(PROPDMG=sum(PROPDMG, na.rm = T),
                              PropDmg=sum(PropDmg, na.rm = T))]
cropdmg <- dts[bad.cropexp, .(CROPDMG=sum(CROPDMG, na.rm = T),
...
The adjustment resulted in `r ceiling(dmg.summary[2,1] / dmg.summary[1,1])`-fold
improvement in property damage estimates, or $`r ceiling((dmg.summary[2,1] - dmg.summary[1,1]) / 1e3)`
billions, and in `r ceiling(dmg.summary[2,2] / dmg.summary[1,2])`-fold
improvement in crop damage estimates,
or $`r ceiling((dmg.summary[2,2] - dmg.summary[1,2]) / 1e3)` billions.
...
dotplot(Element1~econ | Region, data = dt0[econ > 0],
...
dotplot(Element1~humn | Region, data = dt0[humn > 0],
        par.strip.text=list(cex=0.8))
